...
@export var speed: int = 30  # Reducido de 40 a 30 para que sea más lento
@export var gravity: int = 400

@export_category("Combat")
@export var max_health: int = 3  # Más salud que enemy.gd
@export var damage: int = 1
@export var attack_cooldown: float = 1.5  # Cooldown más largo que enemy.gd
@export var attack_distance: float = 80.0  # Distancia a la que el enemigo atacará al jugador (menor que enemy.gd por ser más lento)
...
func patrol() -> void:
	...
	if is_on_wall() or not $RayCast2D.is_colliding():
		...
		print("Enemigo lento cambiando dirección")
...
func _on_hit_area_body_entered(body: Node) -> void:
	if body.is_in_group("player") and can_attack:
		...
		if is_attacking and body.is_in_group("player") and body.has_method("take_damage"):
			print("Enemigo lento atacando al jugador con ", damage, " de daño")
...
func die() -> void:
	...
	set_process(false)
	$CollisionShape2D2.set_deferred("disabled", true)  # Cambiado de CollisionShape2D a CollisionShape2D2
...
func check_attack_distance() -> void:
	...
	if distance <= attack_distance:
		...
		if is_attacking and player_ref and player_ref.has_method("take_damage"):
			print("Enemigo lento atacando al jugador con ", damage, " de daño (por distancia)")
